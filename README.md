...
This scatterplot exhibits the relationship between the actual observed values on the x-axis and the corresponding predictions generated by the model on the y-axis. Each coordinate pair represents the true target variable value from the test set and the predicted estimate respectively.
